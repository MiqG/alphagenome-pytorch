"""Convert a JAX AlphaGenome checkpoint to PyTorch format.

The implementation behind ``agt convert`` and ``scripts/convert_weights.py``.
Lives here rather than in scripts/ so it resolves for wheel installs, and
because it builds directly on this subpackage's weight_mapping/transforms.

Steps:
1. Convert JAX model weights to PyTorch format
2. Extract track_means from JAX metadata and bundle them in the output file
3. Convert weights to native NCL (channels-first) format for Conv1d operations
